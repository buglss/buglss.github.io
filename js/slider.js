var WholePageSlider = class {
  ...
  init() {
    ...
    var sectionButtonContainer = this.createElement('div', { className: 'sectionButtonContainer' }, this.container)

    // Create elements for every section and apply styles
    for(let index = 0, n = this.sections.length; index < n; index++) {
      ...
      if(this.options.colors) {
        var colorList = new Array(n)
        var ci = 0
        for(var i = 0, cs = colorList.length; i < cs; i++) {
          if(!this.options.colors[ci]) ci = 0
          colorList[i] = this.options.colors[ci]
          ci++
        }
        this.sections[index].style.background = colorList[index] ? colorList[index] : 'white'
      }
      ...
      if(n > 1) {
        ...
        var sectionNavigationButton = this.createElement('input', {
          type: 'radio',
      ...
      if(this.pagesPerSection[index].length > 1) {
        ...
        var pageButtonContainer = this.createElement('div', { id: `pageButtonContainer[${index}]`, className: 'page_selection' }, this.sections[index])
  ...
  switchAndTranslateSection(swipeOrClick) {
    ...
    if(((swipeOrClick.deltaY > 0 || swipeOrClick === 'down') && this.swipeStartDirection !== 'up') && (this.currentSection < this.sections.length - 1)) {
      ...
      if(((swipeOrClick.deltaY < 0 || swipeOrClick === 'up') && this.swipeStartDirection !== 'down') && (this.currentSection > 0)) {
        ...
        if(swipeOrClick.type === 'click') {
          var click = parseInt(swipeOrClick.target.value) - this.currentSection
    ...
    var button = document.getElementById(`sectionId[${this.currentSection}]`)
    if(button) {
      button.checked = true
    }
  ...
  switchAndTranslatePage(swipeOrClick) {
    ...
    if(swipeOrClick === 'right' && this.swipeStartDirection !== 'left' && (this.currentPage[this.currentSection] < this.pagesPerSection[this.currentSection].length - 1)) {
      ...
      if(swipeOrClick === 'left' && this.swipeStartDirection !== 'right' && (this.currentPage[this.currentSection] > 0)) {
        ...
        if(swipeOrClick.type === 'click') {
          var getDirectionFromClick = parseInt(swipeOrClick.target.value) - this.currentPage[this.currentSection]
    ...
    // This is needed to show active page on navigation buttons
    var button = document.getElementById(`page[${this.currentSection}][${this.currentPage[this.currentSection]}]`)
    if(button) {
      button.checked = true
    }
  ...
  draggingEffect() {
    ...
    if((this.swipeStartDirection === 'left' || this.swipeStartDirection === 'right') && !this.waitAnimation) {
      ...
      var pages = this.pagesPerSection[this.currentSection]
  ...
  createElement(tag, options, parent) {
    try {
      var getParent = (typeof parent) === 'object' ? parent : document.getElementById(parent)
